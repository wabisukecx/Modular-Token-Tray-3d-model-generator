...
modular_tray_gui.py
==================
modular Tray ジェネレータの PyQt5 GUI。

  python modular_tray_gui.py

modular_tray_generator.py と同じディレクトリに置いて実行してください。
...
# ------------------------------------------------------------
USE_GL = False
if not os.environ.get('modular_GUI_NO_GL'):
    try:
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('modular Tray ジェネレータ')
        self.settings = QSettings('modularTray', 'Generator')
        self.base_box = None
        self.tray = self.lid = None
