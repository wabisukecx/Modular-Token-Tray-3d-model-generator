#!/usr/bin/env python3
"""
modular_tray_generator.py
========================
1x1x1 modular Tray (Storage Box) の3MF/STLを元に、任意サイズ
...
使い方:
  python modular_tray_generator.py --source 1x1x1_Storage_Box.3mf \
      --width 2 --length 3 --height 2 --out-dir ./out
  python modular_tray_generator.py --box box.stl --lid lid.stl -W 2 -L 2 -H 1
...
# ============================================================
def main():
    ap = argparse.ArgumentParser(description='modular Tray ジェネレータ')
    ...
    print(f'=== {W}x{L}x{H} modular Tray 生成{" (区画 "+str(nw)+"x"+str(nl)+")" if tag else ""} ===')
